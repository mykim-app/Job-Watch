...
| 사람인 access-key (선택) | [사람인 API](https://oapi.saramin.co.kr) → 이용신청 → 승인 후 앱 등록 | 심사 있음 |
| 워크넷 인증키 | 워크넷(고용24) 오픈API 신청 | 1~2일 심사 |

- 잡알리오 키는 **일반 인증키(Decoding)** 값을 씁니다. Encoding 값을 넣으면 계속 오류가 납니다.
- 클린아이 잡플러스와 나라일터는 인증키가 필요 없습니다. 그냥 동작합니다.
- 사람인은 없어도 됩니다. 키를 넣으면 그때부터 같이 조회합니다. 호출 한도가 하루 500건이라
  `query_keywords` 를 늘릴 때는 (키워드 수 × max_pages) 가 한도 안에 들어오는지 보세요.
- 워크넷(고용24)은 개인회원에게 채용정보목록 API를 열어주지 않아 사용하지 않습니다.
...
클린아이 잡플러스는 조금 다르게 동작합니다. **모집분야를 '정보통신'으로 지정해 조회**하므로,
"○○도시공사 제6차 기간제근로자 채용" 처럼 제목에 전산이 안 드러난 공고도 잡힙니다.
이렇게 걸린 건 분류가 확실하므로 키워드 필터를 자동 통과합니다.
...
- 서로 다른 수집처에 **같은 공고가 동시에** 올라오면 한쪽만 신규로 처리합니다.
...
notify.py              메일 발송
collectors/
  alio.py              잡알리오 (공공데이터포털, 인증키 필요)
  cleaneye.py          클린아이 잡플러스 (지방공기업·출자출연기관)
  gojobs.py            나라일터 (공무원·공공기관 경력채용)
  saramin.py           사람인 오픈API (인증키 필요, 선택)
