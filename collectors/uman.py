...
거기서 마감일을 뽑아 쓴다. 못 뽑으면 상시로 취급해 화면 정렬에서 맨 뒤로 간다.
"""
...
# "~09. 13(일)" / "~26.09.02" / "~.08.31.(월)" / "~ 9. 3.(목)" / "~09.. 28.(월)" 등
# 구분자(점·하이픈·공백)가 한 개든 여러 개든, 순서만 맞으면 마감일로 인정한다.
# (일부 공고는 "~09.. 28." 처럼 점이 두 번 찍히는 등 표기가 일정하지 않다)
_DEADLINE = re.compile(r"~[.\-\s]*(?:(\d{2})[.\-\s]+)?(\d{1,2})[.\-\s]+(\d{1,2})")
...
# 상세 본문에서 찾을 전산 관련 말.
# '홈페이지'·'정보처리' 는 "홈페이지 참조", "개인정보처리방침" 같은 안내문에도
# 흔해서 오탐이 크므로 뺐다 (인천일자리플랫폼에서 같은 문제를 확인했다).
DEFAULT_IT_WORDS = [
...
def _body_is_it(session, no: str, title: str, words, stops, log, delay) -> bool:
    ...
    return _has_it(body, words, stops)
...
def fetch(cfg: dict, log) -> list[Posting]:
    ...
    for p in out:
        if any(w.lower() in p.title.lower() for w in words):
            p.ncs = "정보통신"
            hit += 1
            continue
